...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # output[:, 1] *= orig_im_dim[0]/inp_dim
            # output[:, 2] *= orig_im_dim[1]/inp_dim
            # output[:, 3] *= orig_im_dim[0]/inp_dim
            # output[:, 4] *= orig_im_dim[1]/inp_dim

            # convert x1, y1, x2, y2 to top, left, bottom, right
            # output[:, 1] = output[:, 4]
            # output[:, 2] = output[:, 1]
            # output[:, 3] = output[:, 2]
            # output[:, 4] = output[:, 3]
